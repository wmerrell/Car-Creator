...
module dreadnaught_end (p) {
  for(yy = [0 : (stringer_thickness(p)*2.75) : car_height(p)-side_sill_lo_height(p)] ) {
...
module end_sill(p, Xpos, Ypos, flip) {
  ...
  if (flip) {
    ...
    translate([Xpos, Ypos+steel_thickness(p), car_height(p)-side_sill_lo_height(p)]) rotate([90, 0, 90]) mirror([0,1,0]) {
      dreadnaught_end(p);
    }
  } else {
    translate([ Xpos-side_sill_thickness(p), Ypos-side_sill_thickness(p), 0]) 
      cube([side_sill_thickness(p)*1.5, car_width(p)+(side_sill_thickness(p)*3), stringer_thickness(p)]);
    translate([Xpos+steel_thickness(p), Ypos+car_width(p)+steel_thickness(p), car_height(p)-side_sill_lo_height(p)]) rotate([90, 0, -90]) mirror([0,1,0]) {
      dreadnaught_end(p);
    }
  }
